...
public class ConnectionHandler {
	...
	private void acct(String info) {
		...
		// The authenticator can also handle special formatted usernames, if really needed (for instance:
		// "username|account")
	...
	private void pasv() throws IOException {
		...
		int nextPort = pasvMinPort;
		while (nextPort <= pasvMaxPort) {
			try {
				passiveServer = Utils.createServer(nextPort++, 5, InetAddress.getByName("0.0.0.0"),
						server.getSSLContext(), secureData);
				break;
		...
		if (passiveServer == null) {
			passiveServer = Utils.createServer(0, 5, InetAddress.getByName("0.0.0.0"),
					server.getSSLContext(), secureData);
		}
		passive = true;
